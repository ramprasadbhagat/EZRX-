...
void main() {
  ...
  group('Payment Info dto ', () {
    ...
    test('PaymentInfoDto fromJson and toDomain on MY market', () {
      final paymentInfoDto =
          PaymentInfoDto.fromJson(dataMY['data']['addCustomerPayment'])
              .toDomain(baseUrl: fakeTHBaseUrl);
      expect(
    ...
    test('PaymentInfoDto fromJson and toDomain on VN market', () {
      final paymentInfoDto =
          PaymentInfoDto.fromJson(dataVN['data']['addCustomerPayment'])
              .toDomain(baseUrl: fakeTHBaseUrl);
      expect(
        paymentInfoDto.zzHtmcs,
    ...
    test(
        'PaymentInfoDto fromJson and toDomain on TH market match regex form action',
        () {
      final paymentInfoDto =
    ...
    test(
        'PaymentInfoDto fromJson and toDomain on TH not match Regex form action',
        () {
      const fakeHtmlFailed =
          '''<html:button id="btn_submit"> Pay Now </html:button><html:script language="javascript">\$("#btn_submit").click(function(){var win, doc;win = window.open("","_blank"); doc = win.document;doc.write('<html><head></head><body><form method="POST" action=/api/thankyou-payment><script type="text/javascript" src="https://dev-kpaymentgateway.kasikornbank.com/ui/v2/kpayment.min.js" data-apikey="pkey_test_21102hvZyafqGFFzN0G3NjphkJZ2NPFbRBMRq" data-amount="32199.51" data-currency="THB" data-payment-methods="qr" data-order-id="order_test_211025257366f3c084154b2941ae1fa149c53"></script></form></body></html>');doc.close();})</html:script>''';
      final paymentInfoDto =
          PaymentInfoDto.fromJson(dataTH['data']['addCustomerPayment'])
              .copyWith(zzHtmcs: fakeHtmlFailed)
              .toDomain(baseUrl: fakeTHBaseUrl);
      expect(
        paymentInfoDto.zzHtmcs,
        '''<html><head></head><body><form method="POST" action=/api/thankyou-payment><script type="text/javascript" src="https://dev-kpaymentgateway.kasikornbank.com/ui/v2/kpayment.min.js" data-apikey="pkey_test_21102hvZyafqGFFzN0G3NjphkJZ2NPFbRBMRq" data-amount="32199.51" data-currency="THB" data-payment-methods="qr" data-order-id="order_test_211025257366f3c084154b2941ae1fa149c53"></script></form></body></html>''',
